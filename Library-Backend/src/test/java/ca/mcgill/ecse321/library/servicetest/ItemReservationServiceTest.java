...
public class ItemReservationServiceTest {
  ...
  public void setMockOutput() {
    ...
    lenient().when(librarySystemRepository.findAll()).thenAnswer((InvocationOnMock invocation) -> {
      List<LibrarySystem> librarySystemList = new ArrayList<LibrarySystem>();
  ...
  */
  ...
  public void testCheckOutReservedItem() {
    assertEquals(0, itemReservationService.getAllItemReservations().size());
    ItemReservation itemReservation = null;
    ItemReservation existingItemReservation = null;
    String error = null;
 ...
 */
  ...
  public void testReservation() {
    ...
    c.set(2021, Calendar.SEPTEMBER, 9, 8, 00, 0);
    try {
      itemReservation =
...
//    try {
//      itemReservation =
//              itemReservationService.reserveItem(person.getId(), reservedItemId, startDate, endDate, startTime, endTime);
//    } catch (IllegalArgumentException e) {
//      error = e.getMessage();
//    }
//    assertNull(itemReservation);
 ...
 */
  ...
  public void testRenew() {
    ...
    itemReservation = itemReservationRepository.findItemReservationById(reservationId);
 ...
 */
  ...
  public void testRenewNonExistingPerson() {
    ...
    ItemReservation itemReservation = null;
    Item item = itemRepository.findItemById(ItemId);
    existingItemReservation = itemReservationRepository.findItemReservationById(reservationId);
  ...
  public void testRenewItemNotCheckedOut() {
    ...
    lenient().when(itemRepository.existsById(item.getId())).thenReturn(true);
    String error = null;
    existingItemReservation = itemReservationRepository.findItemReservationById(testRenewNotCheckedOutItemId);
    lenient().when(itemReservationRepository.existsById(existingItemReservation.getId())).thenReturn(true);
    Date newEndDate = Date.valueOf(LocalDate.parse("2021-12-31"));
    try {
 ...
 */
  ...
  public void testReturnLateTime() {
    ...
    try {
      itemReservation = itemReservationService.returnItem(person.getId(), item.getId(), itemReservation.getId(),
  ...
  public void testReturnItemNotCheckedOut() {
    ...
    lenient().when(personRepository.existsById(person.getId())).thenReturn(true);
    lenient().when(itemRepository.existsById(item.getId())).thenReturn(true);
    itemReservation = itemReservationRepository.findItemReservationById(testRenewNotCheckedOutItemId);
    lenient().when(itemReservationRepository.existsById(itemReservation.getId())).thenReturn(true);
    Date newEndDate = Date.valueOf(LocalDate.parse("2021-12-22"));
    try {
      itemReservation =
 ...
 */
  ...
  public void testDeleteReservation() {
    ...
    assertEquals(null, itemReservationRepository.findItemReservationByPersonAndItemAndTimeSlot(person, item,
